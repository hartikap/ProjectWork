...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (!isDigit(incomingByte)) {
      ...
      if (incomingByte == 'A') {
        writeVoltage(channel, inString.toInt());
        // clear the string for new input:
        inString = "";
        channel = 'X'; // default channel - points nowhere
      } else {
        channel = incomingByte; // other characters define the output channel
      }
...
void writeVoltage(char ch,int value) {
  switch(ch) {
    case 'B': 
